...
class AudioSegmentDownloader(private val context: Context) {
    ...
    fun cutSegmentFromLocal(
        localSourcePath: String,
        startSec: Int,
        endSec: Int,
        outputProfile: OutputProfile = OutputProfile.COMPRESSED_WEBM
    ): File {
        ...
        val ext = if (outputProfile == OutputProfile.WAV_16K_MONO) "wav" else "webm"
        val outFile = File(outputDir, "seg-${System.currentTimeMillis()}-${startSec}s-${endSec}s.$ext")
        ...
        val cmd = when (outputProfile) {
            OutputProfile.COMPRESSED_WEBM -> buildString {
            ...
            OutputProfile.WAV_16K_MONO -> buildString {
                append("-y -hide_banner -loglevel error -nostdin ")
                append("-ss $coarseSeekSec -i \"$localIn\" -ss $fineSeekSec -t $durationSec -vn ")
